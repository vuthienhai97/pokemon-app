...
// ─── Public service functions (called server-side, with Next.js revalidate) ───

export async function fetchTypes(): Promise<TypeInfo[]> {
  const res = await fetch(`${BASE}/type?limit=100`, { next: { revalidate: 86400 } })
...
async function fetchByTypes(
  ...
  const responses = await Promise.all(
    types.map((t) =>
      fetch(`${BASE}/type/${t}`, { next: { revalidate: 3600 } }).then(
...
async function fetchDetail(nameOrId: string | number): Promise<Pokemon> {
  const res = await fetch(`${BASE}/pokemon/${nameOrId}`, { next: { revalidate: 3600 } })
